...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Initialize weights with a normal distribution and biases with zeros
         ...
         # If normalization is being used, initialize scale (gamma) and shift (beta) parameters
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            # Initialize required variables for first layer.
            if i == 1:
                o, c, names = [X], [], []                  # o[0]: X for initial step
            else:
                o, c, names = [iter_out], [], []    # o[0]: output from the previous iteration
    
            # Affine forward pass (linear transformation).
            a, a_c = affine_forward(o[-1], self.params['W'+str(i)], self.params['b'+str(i)])
            o.append(a), c.append(a_c), names.append('a'+str(i))
    
            # Batch normalization forward pass if required.
            if self.normalization == "batchnorm":
            ...
            # Layer normalization forward pass if required.
            if self.normalization == "layernorm":
            ...
            # Dropout forward pass if required.
            if self.use_dropout:
                d, d_c = dropout_forward(o[-1], self.dropout_param)
                o.append(d), c.append(d_c), names.append('d'+str(i))
    
            outs.update(list(zip(names, o[1:])))    # save the outputs excluding the initial input layer
            caches.update(list(zip(names, c)))
    
            iter_out = o[-1]
    
        # Affine forward pass for the output layer.
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # Implementing the backward pass of the network.
        ...
        # Iterate over layers in reverse order to compute gradients.
        for i in range(self.num_layers -1, 0, -1):
    
            # Dropout backward pass if required.
            if self.use_dropout:
            ...
            # Batch normalization backward pass if required.
            if self.normalization == "batchnorm":
                dout, grads['gamma'+str(i)], grads['beta'+str(i)] = batchnorm_backward_alt(dout, caches['n'+str(i)])
            
            # Layer normalization backward pass if required.
            if self.normalization == "layernorm":
