...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Compute the next hidden state using a tanh activation function
    next_h = np.tanh(np.dot(prev_h, Wh) + np.dot(x, Wx) + b)
    # Store variables needed for the backward pass
...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = 1 - np.square(next_h)  # Derivative of tanh.
    dnext_h_dtanh = dnext_h * dtanh
    
    # Compute gradients with respect to input data, previous hidden state, weights, and biases.
    dx = np.dot(dnext_h_dtanh, Wx.T)
    dprev_h = np.dot(dnext_h_dtanh, Wh.T)
    dWx = np.dot(x.T, dnext_h_dtanh)
    dWh = np.dot(prev_h.T, dnext_h_dtanh)
    db = np.sum(dnext_h_dtanh, axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # Initialize the array for the hidden states and cache
    N, T, D = x.shape
    N, H = h0.shape
...
def rnn_backward(dh, cache):
    ...
    # Retrieve the shapes and initialize gradients.
    N, T, H = dh.shape
    D, _ = cache[0][1].shape  # D is derived from Wx shape in cache
    dx = np.zeros([N, T, D]) 
    dh0 = np.zeros([N, H]) 
    dWx = np.zeros([D, H]) 
    dWh = np.zeros([H, H]) 
    ...
    for i in reversed(range(T)):
        # Accumulate the gradient from the next time step.
        # dh[:, i, :] is the gradient from the loss at time step i.
        # dh0 is the gradient being passed from the next time step.
        dnext_hi = dh[:, i, :] + dh0
        ...
        # Accumulate the gradients for input, weights, and biases.
        dx[:, i, :] = dxi
        dWx += dWxi
        dWh += dWhi
        db += dbi 
    
        # Update the hidden state gradient to be passed to the previous time step.
        dh0 = dprev_hi
...
def word_embedding_forward(x, W):
    ...
    # Compute the output word and store values 
    out, cache = W[x], (x, W)
...
def word_embedding_backward(dout, cache):
    ...
    # Unpack cache and initialize gradient 
    #of word embedding matrix
    x, W = cache
    dW = np.zeros_like(W)
    # Accumulate gradients for the word embedding matrix.
    np.add.at(dW, x, dout)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # Computing the intermediate step (A) by multiplying inputs and adding bias.
    A = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    
    # Slicing A to obtain gates' activations.
    H = np.int(A.shape[1] / 4)
    ai, af, ao, ag = A[:, :H], A[:, H:2*H], A[:, 2*H:3*H], A[:, 3*H:]
    
    # Applying non-linear activations (sigmoid and tanh) on gates.
    ...
    # Storing the required values for backward pass in cache.
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # Calculating the gradients of next_h and next_c.
    dl_dnexth = dnext_h
    dnexth_do = np.tanh(next_c)
    do_dao = o * (1 - o)
    dl_dao = dl_dnexth * dnexth_do * do_dao
    
    # Handling the gradient through the tanh nonlinearity.
    dnexth_dnextc = 1 - np.square(np.tanh(next_c))
    dl_dnextc = dnext_c + o * dl_dnexth * dnexth_dnextc
    
    # Calculating gradients for input, forget, and output gates.
    dnextc_di, di_dai, dnextc_df, df_daf = g, i * (1 - i), prev_c, f * (1 - f)
    dnextc_dg, dg_dag = i, 1 - np.square(g)
    dl_dai, dl_daf, dl_dag = dl_dnextc * dnextc_di * di_dai, dl_dnextc * dnextc_df * df_daf, dl_dnextc * dnextc_dg * dg_dag
    
    # Computing gradients for the previous cell state.
    dnextc_dprevc = f
    dl_dprevc = dl_dnextc * dnextc_dprevc
    
    # Concatenating gradients for the activation vector.
    dl_da = np.concatenate((dl_dai, dl_daf, dl_dao, dl_dag), axis=1)
    
    # Calculating gradients for weights and input.
    dl_dwx, dl_dwh, dl_dx, dl_dprevh = np.dot(x.T, dl_da), np.dot(prev_h.T, dl_da), np.dot(dl_da, Wx.T), np.dot(dl_da, Wh.T)
    
    # Updating gradients for dx, dprev_h, dprev_c, dWx, dWh, and db.
    dx, dprev_h, dprev_c, dWx, dWh, db = dl_dx, dl_dprevh, dl_dprevc, dl_dwx, dl_dwh, np.sum(dl_da, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    # Initializing cell state, list for hidden states and cache.
    c, hs, cache = np.zeros_like(h0), [h0], []
    
    # Iterating over time steps.
    for t in range(x.shape[1]):
        # Forward step for each time step.
        h, c, cache_t = lstm_step_forward(x[:, t], hs[-1], c, Wx, Wh, b)
        hs.append(h)
        cache.append(cache_t)
    
    # Stacking hidden states along time axis.
...
def lstm_backward(dh, cache):
    ...
    # Initialize gradients and dimensions.
    ...
    # Initialize gradient for next cell state.
    dnext_c = np.zeros([N, H])
    
    # Process each timestep in reverse order for backpropagation.
    for i in reversed(range(T)):
        # Select the appropriate gradient of next hidden state.
        dnext_hi = dh[:, i, :] + (dh0 if i < T - 1 else 0)
    
        # Perform a backward step using the cached values.
        cachei = cache[i]
        dxi, dprev_hi, dprev_c, dWxi, dWhi, dbi = lstm_step_backward(dnext_hi, dnext_c, cachei)
    
        # Accumulate gradients from the current step.
        dnext_c = dprev_c
        dx[:, i, :] = dxi
        dWx += dWxi
        dWh += dWhi
